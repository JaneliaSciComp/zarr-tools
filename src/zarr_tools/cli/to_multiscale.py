# This code comes from Yurii Zubov's zarr-multiscale github repo 
# at https://github.com/janelia-cellmap/zarr-multiscale.git
...
def create_multiscale(z_root: zarr.Group, level0_subpath:str, client: Client, data_origin: str, antialiasing : bool):
    """
    ...
    level = 1
    source_shape = z_root[level0_subpath].shape
    ...
    while all([dim > 32 for dim in spatial_shape]):
        print(f'level={level}')
        if level == 1:
            source_arr = z_root[level0_subpath]
        else:
...
@click.option('--subpath', type=click.STRING, help='Input .zarr level0 dataset location.')
@click.option('--workers','-w', default=10,type=click.INT, help = "Number of dask workers")
